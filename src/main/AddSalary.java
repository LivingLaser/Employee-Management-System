...
public class AddSalary implements InsertSalary {
	...
	private String e_id_1, e_id_2, basic, da, hra, ma, cca, pf, gross, net;
	private double num_1, num_2, num_3, num_4, num_5, num_6, res_1, res_2, res_3;
	
	AddSalary() {
		
		frame_1 = new JFrame("EMS - Set Salary");
		frame_2 = new JFrame("EMS - Set Salary");
		...
		heading = new JLabel("New Salary Details");
		...
		frame_2.add(heading);
		heading.setBounds(370, 10, 330, 50);
	...
	public void setSalary() {
		...
		try {
			...
			PreparedStatement pstm_2 = con.prepareStatement("select e_id from salary where e_id=?");
			pstm_2.setString(1, e_id_1);
			PreparedStatement pstm_3 = con.prepareStatement("select e_id, name, email_id, phone from employee where e_id=?");
			pstm_3.setString(1, e_id_1);
			ResultSet rs_2, rs_3;
			...
			try {
				if(rs_1.next()) {
					...
					if(rs_2.next()) {
						JOptionPane.showMessageDialog(frame_1, "Record already exist", "Warning", JOptionPane.WARNING_MESSAGE);
						text_1.setText(null);
					}
					else {
						if(rs_3.next()) {
							label_text_1.setText(rs_3.getString(1));
							label_text_2.setText(rs_3.getString(2));
							label_text_3.setText(rs_3.getString(3));
							label_text_4.setText(rs_3.getString(4));
						}
						frame_1.dispose();
					...
					JOptionPane.showMessageDialog(frame_1, "Employee record not found", "Warning", JOptionPane.WARNING_MESSAGE);
					text_1.setText(null);
				}
			}
			finally {
			...
			JOptionPane.showMessageDialog(frame_1, "Failed to insert data", "Error", JOptionPane.ERROR_MESSAGE);
		}
	}
	...
	public void getSalary() {
		...
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/ems", "root", "DBMS");
			PreparedStatement pstm = con.prepareStatement("insert into salary values (?, ?, ?, ?, ?, ?, ?, ?, ?)");
			...
			try {
				if(basic.isBlank() || da.isBlank() || hra.isBlank() || ma.isBlank() || cca.isBlank() || pf.isBlank()) {
					...
					pstm.setString(1, e_id_2);
					pstm.setString(2, basic);
					pstm.setString(3, da);
					pstm.setString(4, hra);
					pstm.setString(5, ma);
					pstm.setString(6, cca);
					pstm.setString(7, pf);
					pstm.setString(8, gross);
					pstm.setString(9, net);
					pstm.executeUpdate();
					
					JOptionPane.showMessageDialog(frame_2, "Record inserted successfully", "Success", JOptionPane.INFORMATION_MESSAGE);
					frame_2.dispose();
				}
			}
			finally {
			...
			JOptionPane.showMessageDialog(frame_2, "Failed to insert data", "Error", JOptionPane.ERROR_MESSAGE);
		}
	}
